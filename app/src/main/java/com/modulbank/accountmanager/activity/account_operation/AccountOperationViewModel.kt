...
import com.modulbank.accountmanager.models.accounts.TransferModel
...
class AccountOperationViewModel : ViewModel() {
    ...
    fun transfer(userDao : UserDao, accountApi  :IAccountApi, accountId : String, amount : BigDecimal, number : Long, currency : Int) {
        val user = userDao.Get() ?: return

        state.value = AccountOperationState(isLoading = true)

        accountApi.transfer("Bearer ${user.value}", accountId, TransferModel(number, amount, currency))
            .subscribeOn(Schedulers.io())
            ?.observeOn(AndroidSchedulers.mainThread())
            .subscribe(
                {
